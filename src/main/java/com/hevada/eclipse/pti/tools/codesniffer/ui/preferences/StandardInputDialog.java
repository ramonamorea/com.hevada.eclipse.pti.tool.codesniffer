...
public class StandardInputDialog extends StatusDialog {
	...
	public StandardInputDialog(Shell parent, Standard standard, List<Object> list) {
		super(parent);

		fExistingNames = new ArrayList<String>(list.size());
		for (int i = 0; i < list.size(); i++) {
			Standard curr = (Standard) list.get(i);
	...
	private void doValidation() {
		...
		if (newPath != null && newPath.length() > 0) {
			...
			if (path != null) {
				File codingStandardPHP = new File(
					newPath + File.separatorChar + path.lastSegment() + "CodingStandard.php");
				File codingStandardXML = new File(newPath + File.separatorChar + "ruleset.xml");
				File customXML = new File(newPath);
				if (!codingStandardPHP.exists() && !codingStandardXML.exists() && !customXML.exists()
					&& !newPath.endsWith(".xml")) {
					status
						.setError("Missing standard file " + path.lastSegment() + "CodingStandard.php or ruleset.xml");
				}
			}
		}
